 ...
 * @brief Driver implementation for the ESP32-compatible Incremental Rotary
 * Encoder component.
 *
 ...
 * - Then both bits are high, halfway through the step.1
...
// Use a single-item queue so that the last value can be easily overwritten by
...
void RotaryEncoderButton::enable_half_steps(bool enable) {

  info.table = enable ? &_ttable_half[0] : &_ttable_full[0];
  info.table_state = R_START;
}

void RotaryEncoderButton::flip_direction() {
...
void RotaryEncoderButton::uninit() {
  detachInterrupt(info.pin_a);
  detachInterrupt(info.pin_b);
}

QueueHandle_t RotaryEncoderButton::create_queue(void) {
  return xQueueCreate(EVENT_QUEUE_LENGTH, sizeof(rotary_encoder_event_t));
}

void RotaryEncoderButton::set_queue(QueueHandle_t queue) { info.queue = queue; }

esp_err_t RotaryEncoderButton::get_state(rotary_encoder_state_t *state) {
...
void RotaryEncoderButton::reset() {
...
esp_err_t RotaryEncoderButton::init(gpio_num_t pin_a, gpio_num_t pin_b,
  ...
  info.pin_a = pin_a;
  info.pin_b = pin_b;
  info.table = &_ttable_full[0]; // enable_half_step ? &_ttable_half[0] :
                                 // &_ttable_full[0];
  info.table_state = R_START;
  info.state.position = 0;
  info.state.direction = ROTARY_ENCODER_DIRECTION_NOT_SET;

  pinMode(info.pin_a, INPUT_PULLUP);
  pinMode(info.pin_b, INPUT_PULLUP);
  attachInterruptArg(info.pin_a, isr_rotenc_, this, CHANGE);
  attachInterruptArg(info.pin_b, isr_rotenc_, this, CHANGE);


  enable_half_steps(enable_halfsteps);
  event_queue = create_queue();
  set_queue(event_queue);
  // Watch the queue and call  registered callback if we have new data
          ...
          log_v(
              ...
              event.state.direction
                  ? (event.state.direction == ROTARY_ENCODER_DIRECTION_CLOCKWISE
                         ...
                         : "CCW")
                  : "NOT_SET");
          if (me->on_rotary_event) {
            me->on_rotary_event(event);
          }
        }
      }
    }
...
IRAM_ATTR uint8_t RotaryEncoderButton::process_(RotaryEncoderButton *encoder) {
  ...
  if (info != NULL) {
    ...
    uint8_t pin_state =
        (gpio_get_level(info->pin_b) << 1) | gpio_get_level(info->pin_a);
...
// Determine new state from the pins and state table.
    ...
    ESP_EARLY_LOGD(TAG, "BA %d%d, state 0x%02x, new state 0x%02x, event 0x%02x",
                   pin_state >> 1, pin_state & 1, old_state, info->table_state,
                   event);
#endif
  }
  return event;
...
IRAM_ATTR void RotaryEncoderButton::isr_rotenc_(void *this_ptr) {
  ...
  if (encoder->fast_interval_ > 0 &&
      millis() - lastevent < encoder->fast_interval_) {
    // Speed up when turning faster
    increment = 2;
    if (encoder->veryfast_interval_ &&
        millis() - lastevent < encoder->veryfast_interval_) {
      // Speed up when turning faster
      increment = 4;
    }
  }
        ...
        .state =
            {
                ...
                .speed = static_cast<rotary_encoder_speed_t>(increment),
            },
    };
    lastevent = millis();
